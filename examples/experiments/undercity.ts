...
(async function main() {
  ...
  const mapExportConfig: MapExportConfig = {
    ...
    max: [1, 1],
    mapAngleDeg: 90,
    wmoSet: ['world\\wmo\\lorderon\\undercity\\undercity_set0.obj'],
    terrain: {
      clampPercent: {
        lower: 0,
        upper: 0.2,
  ...
  const mapDir = './maps/undercity-indoor.w3x';
  ...
  const undercityType = sourceManager.doodadTypes.find((dt) => dt.data.some((d) => d.id === 'dfil' && (d.value as string).includes('undercity_set0')))!;

  const undercity = sourceManager.doodads.find((d) => d.type === undercityType)!;

  undercityType.data.find((d) => d.id === 'dfil')!.value = 'model.mdl';

  const ground = await buildUndercityLower(exporter.wowObjectManager.assetManager, mapDir);
  matchTerrainToDoodadHeights(sourceManager.mapData.terrain, [
    [undercity, ground.model.toMdl()],
  ], {
    floodBrushSize: 10,
    slopeThreshold: 30,
  });

  sourceManager.doodads = sourceManager.doodads.filter((d) => d.position[2] < 7000 || d.type === undercityType);
...
async function buildUndercityLower(assetManager: AssetManager, outputPath: string) {
  const model = (await assetManager.parse('world\\wmo\\lorderon\\undercity\\undercity_set0.obj', true)).mdl;
