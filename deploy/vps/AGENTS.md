...
- Host `wow-converter` + `wow.export` on a VPS behind nginx, managed by **systemd**.
- Live scripts live in **`/root/wow-hosting/`**; this git folder is the source of truth.
- Resilience: boot automatically, restart on failure, clear NW.js profile locks on wow.export start.
...
Services: `wow-export` (:17752 REST, Xvfb) → `wow-converter` (:3001). nginx proxies :443 → 3001.
...
```bash
apt update && apt install -y git nginx certbot python3-certbot-nginx xvfb curl python3 make g++
curl -fsSL https://bun.sh/install | bash

cd /root
git clone --recursive https://github.com/pqhuy98/wow-converter.git
cd wow-converter
cp deploy/vps/env.example .env
bun install && bun run build:server
( cd wow.export && bun install && bun run build-linux )

bash deploy/vps/sync-from-repo.sh
systemctl start wow-export wow-converter
...
systemctl status wow-export wow-converter
journalctl -u wow-converter -f
systemctl start wow-hosting-maintenance.service        # manual clean + restart
```

Verify: `curl -sf http://127.0.0.1:3001/` and `curl -sf http://127.0.0.1:17752/rest/getConfig`

## Deploy flow (`deploy-app.sh`)

1. `git fetch` + `reset --hard origin/main` + submodule update (skip if unchanged)
2. `sync-from-repo.sh`
3. `bun install` + `bun run build:server`
4. Rebuild `wow.export` linux binary **only if submodule SHA changed**
5. Restart `wow-export`, then `wow-converter`

## Troubleshooting

**Profile lock** — cleared automatically on wow.export start. Manual fix:

```bash
systemctl stop wow-export
rm -f /root/.config/wow.export-huy-edition/SingletonLock \
      /root/.config/wow.export-huy-edition/SingletonSocket \
      /root/.config/wow.export-huy-edition/SingletonCookie
systemctl start wow-export
```

**502 / not ready** — CASC load still running; check `journalctl -u wow-converter`.

**Disk** — weekly `clean.sh` removes `~/wow.export`, `exported-assets`, `exported-assets-browse`, `recent-exports.json`. Not the `wow.export` submodule.
