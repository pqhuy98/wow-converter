...
import { Config, WowObject } from './common';
...
export class Wc3Converter {
  ...
  generateTerrainWithHeight(terrainObjs: WowObject[], options: Config): Terrain {
  ...
  placeDoodads(roots: WowObject[], terrain: Terrain, options: Config, filter: (obj: WowObject) => boolean = () => true) {
    ...
    const placeDoodadsRecursive = (obj: WowObject, parentAbsolute: WowObject | null) => {
      ...
      if (filter(obj)) {
        ...
        if (!modelPathToId.has(hashKey)) {
          ...
          const doodadName = `~ ${path.basename(obj.model!.relativePath)} -- ${obj.type} -- ${fullId.slice(0, 4)}`;
        ...
        const zDiff = (dataHeightMax - dataHeightMin) * (percent[2] - options.terrainHeightClampPercent.lower) / (options.terrainHeightClampPercent.upper - options.terrainHeightClampPercent.lower);
    ...
    return { doodadsData, doodads, doodadTypesWithPitchRoll };
  }
}

function computeTerrainHeightMap(terrains: WowObject[], options: Config) {
