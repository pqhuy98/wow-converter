...
import { getWacraftSequenceData, getWowAnimName, isLoopAnimation } from '@/lib/converter/wow-model/animation/animation-mapper';
import { getBoneName } from '@/lib/converter/wow-model/animation/bones-mapper';
import { wowToWc3Interpolation } from '@/lib/converter/wow-model/bundle/utils';
...
export class AnimationFile implements AnimationData {
  ...
  /** Populate from an already-built bones data object (direct M2 pipeline). */
  loadFromData(data: AnimationData & { attachments?: Data.Attachment[] }): this {
  ...
  async parse(): Promise<this> {
    try {
      ...
      Object.assign(this, JSON.parse(await readExportAssetUtf8(this.filePath)));
