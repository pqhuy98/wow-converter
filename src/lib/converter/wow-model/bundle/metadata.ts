...
import { BlizzardNull } from '@/lib/constants';
import { AnimationFile } from '@/lib/converter/wow-model/bundle/animation';
import { getLayerFilterMode, wmoBlendModeToWc3FilterMode, wowToWc3Interpolation } from '@/lib/converter/wow-model/bundle/utils';
...
export class M2MetadataFile {
  ...
  constructor(private filePath: string, private config: Config, private animFile: AnimationFile) {
  }

  /** Attach the target MDL before any extract* call. */
  bindMdl(mdl: MDL): this {
    this.mdl = mdl;
    this.globalSequenceMap = new Map<number, GlobalSequence>(mdl.globalSequences.map((gs) => [gs.id, gs]));
    return this;
  }

  /** Populate from an already-parsed metadata object (direct M2 pipeline). */
  loadFromData(raw: Record<string, unknown>): this {
    this.applyRaw(raw);
    return this;
  }
  ...
  async parse(): Promise<this> {
    try {
      ...
      const raw = JSON.parse(await readExportAssetUtf8(this.filePath));
      this.applyRaw(raw);
    } catch (e) {
      if (e.code === 'ENOENT') {
  ...
  private applyRaw(raw: Record<string, unknown>): void {
    this.fileType = raw.fileType as string;
    if (this.fileType === 'm2') {
      Object.assign(this, raw);
      this.isLoaded = true;
    } else if (this.fileType === 'wmo') {
      // Selectively assign fields for WMO JSON
      this.fileDataID = raw.fileDataID as number;
  ...
  extractMDLParticlesEmitters(textures: Texture[]) {
    ...
    const random = seededRandom(`particle-emitters:${this.mdl.model.name}`);
    ...
    this.particleEmitters.forEach((p, i) => {
      ...
      for (let i = 0; i < variantCount; i++) {
        ...
        if (chooseRandomTexture) {
          const cell = Math.floor(random() * p.textureRows * p.textureCols);
        ...
        if (p.lifespanVary >= varyThreshold) {
          const newLifeSpan = variant.lifeSpan + random() * p.lifespanVary;
