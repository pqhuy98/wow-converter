...
export function addMdlItemToBone(this: MDLModify, item: MDL, boneName: string) {
  ...
  console.log(`Attaching item "${path.basename(item.model.name)}" to bone "${attachmentBone.name}"...`);
...
export function addMdlCollectionItemToModel(this: MDLModify, item: MDL) {
  console.log(`Attaching item "${path.basename(item.model.name)}" as collection...`);
