/**
 * Faithful TypeScript port of wow.export's jenkins96 lookup3 hash (src/js/casc/jenkins96.js).
 * Based off original works by Robert John Jenkins Junior (port by bryc).
 */
export function jenkins96(k: Uint8Array | Buffer | number[], init = 0, init2 = 0): [number, number] {
  let len = k.length;
  let o = 0;
  let a = (0xDEADBEEF + len + init) | 0;
  let b = (0xDEADBEEF + len + init) | 0;
  let c = (0xDEADBEEF + len + init + init2) | 0;
