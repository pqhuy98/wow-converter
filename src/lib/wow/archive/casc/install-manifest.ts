/**
 * Install manifest parser, ported from wow.export (src/js/casc/install-manifest.js).
 */
import type { BufferWrapper } from '../../formats/buffer';

const INSTALL_SIG = 0x4E49; // IN

export interface InstallTag {
  name: string;
  type: number;
  mask: number[];
}
...
export class InstallManifest {
  ...
  parse(data: BufferWrapper): void {
    if (data.readUInt16LE() !== INSTALL_SIG) throw new Error('Invalid file signature for install manifest');
    ...
    for (const tag of this.tags) {
      ...
      for (let i = 0, n = mask.length; i < n; i++) {
        for (let j = 0; j < 8; j++) {
          if (((mask[i] >>> (7 - j)) & 0x1) === 1) this.files[((i % n) * 8) + j]?.tags.push(tag.name);
        }
      }
    }
  }
