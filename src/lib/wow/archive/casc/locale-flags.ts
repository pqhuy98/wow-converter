/**
 * CASC Locale Flags, ported from wow.export (src/js/casc/locale-flags.js).
 */
export const LocaleFlags = {
  enUS: 0x2,
  koKR: 0x4,
  frFR: 0x10,
  deDE: 0x20,
  zhCN: 0x40,
  esES: 0x80,
  zhTW: 0x100,
  enGB: 0x200,
  esMX: 0x1000,
  ruRU: 0x2000,
  ptBR: 0x4000,
  itIT: 0x8000,
  ptPT: 0x10000,
} as const;

export type LocaleFlagName = keyof typeof LocaleFlags;

export const LocaleNames: Record<LocaleFlagName, string> = {
  enUS: 'American English [enUS]',
  koKR: '한국어 [koKR]',
  frFR: 'Français [frFR]',
  deDE: 'Deutsch [deDE]',
  zhCN: '简体中文 [zhCN]',
  esES: 'Español (España) [esES]',
  zhTW: '繁體中文 [zhTW]',
  enGB: 'British English [enGB]',
  esMX: 'Español (América Latina) [esMX]',
  ruRU: 'Русский [ruRU]',
  ptBR: 'Português (Brasil) [ptBR]',
  itIT: 'Italiano [itIT]',
  ptPT: 'Português (Europeu) [ptPT]',
};

export default LocaleFlags;
