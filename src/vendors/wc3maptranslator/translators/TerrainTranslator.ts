...
function splitLargeArrayIntoWidthArrays<T>(array: T[], width: number): T[][] {
  const rows: T[][] = [];
...
export class TerrainTranslator implements Translator<Terrain> {
  ...
  public static getInstance(): TerrainTranslator {
    if (!this.instance) {
  ...
  public warToJson(buffer: Buffer): JsonResult<Terrain> {
    ...
    /**
         * Header
         */
    const _w3eHeader = outBufferToJSON.readChars(4); // W3E!
    ...
    while (!outBufferToJSON.isExhausted()) {
      ...
      const flagsAndGroundTexture = version >= 12 ? outBufferToJSON.readShort() : outBufferToJSON.readByte();
      ...
      const flags = flagsAndGroundTexture & (version >= 12 ? 0xffc0 : 240);
      const groundTexture = flagsAndGroundTexture & (version >= 12 ? 0x3f : 15);
    ...
    result.flags = splitLargeArrayIntoWidthArrays(arrFlags, result.map.width + 1);
