...
// CSS color mapping
const CSS_COLORS = {
  // Use neutral tokens so they are visible on dark terminal
  ...
  cyan: 'text-cyan-600',
  white: 'text-[hsl(var(--terminal-fg))]',
  ...
  brightWhite: 'text-[hsl(var(--terminal-fg))]',
  bgBlack: 'bg-[hsl(var(--terminal-bg))]',
  bgRed: 'bg-red-600',
  ...
  bgCyan: 'bg-cyan-600',
  bgWhite: 'bg-card',
  bgBrightBlack: 'bg-[hsl(var(--terminal-bg))]',
...
export function Terminal({ logs, className = '' }: TerminalProps) {
  return (
    <div className={`bg-[hsl(var(--terminal-bg,_var(--secondary)))] text-[hsl(var(--terminal-fg))] font-mono text-sm p-4 rounded-lg border border-border max-h-40 overflow-y-auto ${className}`}>
          ...
          return (
            <div key={index} className="flex overflow-hidden whitespace-nowrap">
              {segments.map((segment, segIndex) => {
                const classes = [
                  segment.color || 'text-[hsl(var(--terminal-fg))]',
